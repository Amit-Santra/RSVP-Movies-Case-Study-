...
SELECT year, COUNT(id) as total_movies_released
FROM movie
GROUP BY year
ORDER BY year;
...
SELECT genre, COUNT(movie_id) AS number_of_movies_2019
FROM genre AS g
INNER JOIN movie AS m
ON g.movie_id = m.id
WHERE year = 2019
GROUP BY genre
ORDER BY number_of_movies_2019 DESC
LIMIT 1;
...
WITH one_genre AS 
(SELECT movie_id, COUNT(genre) AS genre
FROM genre
GROUP BY movie_id
HAVING genre = 1
)
...
SELECT total_votes, country
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
WHERE country LIKE '%Germany%' OR country LIKE '%Italy%'
GROUP BY total_votes, country
ORDER BY total_votes DESC;

-- CONSIDERING ORIGIN COUNTRY OF THE MOVIE
...
(SELECT n.NAME AS actress_name,
		Sum(r.total_votes) AS total_votes,
        Count(DISTINCT rm.movie_id) AS movie_count,
        Round(Sum(r.avg_rating * r.total_votes) / Sum(r.total_votes), 2) AS actess_avg_rating
FROM   role_mapping AS rm
INNER JOIN names AS n                   
ON rm.name_id = n.id                
INNER JOIN ratings AS r                
...
SELECT *,  
Rank()  OVER ( ORDER BY actess_avg_rating DESC) AS aactess_rank                  
FROM   top_actess 
LIMIT 5;
...
top_5_movies AS (
	SELECT genre, m.year, m.title AS movie_name, m.worlwide_gross_income,
...
AND  POSITION(',' IN languages)>0
 -- there is null values in production_company which has 10 such movies with 8+ ratings
...
WITH actress_ratings AS (
   ...
   INNER JOIN genre AS g
   ON m.id = g.movie_id
   WHERE rm.category = 'actress' AND lower(g.genre) = 'drama'
   AND r.avg_rating > 8
...
movie_summary AS (
   SELECT n.id AS director_id, n.name AS director_name,
      m.id AS movie_id, m.date_published, m.duration,
